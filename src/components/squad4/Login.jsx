...
const Login = () => {
    ...
    const handleLogin = async () => {
        ...
        try {
            const response = await fetch('http://43.204.228.125:8080/api/users/login', {
                method: 'POST',
                headers: {
    ...
    const handleSendOtp = async () => {
        if (captchaVerified) {
            ...
            try {
                const response = await fetch(`http://43.204.228.125:8080/api/users/send-otp/${normalizedEmail}`, {
                    method: 'POST',
                });
